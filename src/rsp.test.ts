...
function makeQuad(id: string, predicate = 'http://rsp.js/test_property', object = 'http://rsp.js/test_object') {
    return quad(
        namedNode(`https://rsp.js/${id}`),
        namedNode(predicate),
        namedNode(object),
        defaultGraph(),
    );
...
function waitForResults(emitter: EventEmitter, expected: number): Promise<binding_with_timestamp[]> {
    return new Promise((resolve, reject) => {
        ...
        const onResult = (result: binding_with_timestamp) => {
            results.push(result);
...
test('rsp_consumer_test', async () => {
    const rspEngine = new RSPEngine(singleWindowQuery());
...
test('rsp_multiple_different_window_test', async () => {
    const query = `PREFIX : <https://rsp.js/>
    REGISTER RStream <output> AS
    SELECT *
    FROM NAMED WINDOW :w1 ON STREAM :stream1 [RANGE 10 STEP 2]
...
test('normal in-order input preserves max_delay zero behavior and trailing timestamps', async () => {
    const rspEngine = new RSPEngine(singleWindowQuery('10 STEP 10'), { max_delay: 0 });
    const stream = rspEngine.getStream("https://rsp.js/stream1");
    const emitter = rspEngine.register();
    const resultsPromise = waitForResults(emitter, 2);

    stream?.add(new Set([makeQuad('in-order-0')]), 0);
    stream?.add(new Set([makeQuad('in-order-1')]), 1);
    stream?.add(new Set([makeQuad('in-order-10')]), 10);
...
test('accepts an event exactly at max_delay and delays output until the watermark closes the window', async () => {
    const rspEngine = new RSPEngine(singleWindowQuery('10 STEP 10'), { max_delay: 5 });
    const stream = rspEngine.getStream("https://rsp.js/stream1");
    const emitter = rspEngine.register();
    const resultsPromise = waitForResults(emitter, 2);

    stream?.add(new Set([makeQuad('bounded-0')]), 0);
    stream?.add(new Set([makeQuad('bounded-10')]), 10);
    ...
    stream?.add(new Set([lateQuad]), 5);
    expect(rspEngine.windows[0].time).toBe(10);
    expect(rspEngine.windows[0].get_current_watermark()).toBe(5);

    stream?.add(new Set([makeQuad('bounded-15')]), 15);
    const results = await resultsPromise;
    expect(results).toHaveLength(2);
...
test('rejects an event beyond max_delay through the public RDFStream API', async () => {
    const rspEngine = new RSPEngine(singleWindowQuery('10 STEP 10'), { max_delay: 5 });
    const stream = rspEngine.getStream("https://rsp.js/stream1");
    const emitter = rspEngine.register();
    const resultsPromise = waitForResults(emitter, 1);

    stream?.add(new Set([makeQuad('rejected-0')]), 0);
    stream?.add(new Set([makeQuad('rejected-10')]), 10);
    stream?.add(new Set([makeQuad('too-late')]), 4);
    stream?.add(new Set([makeQuad('rejected-15')]), 15);
...
test('surfaces query processing errors through the engine emitter', async () => {
    const rspEngine = new RSPEngine(singleWindowQuery('10 STEP 10'));
    const stream = rspEngine.getStream("https://rsp.js/stream1");
    const emitter = rspEngine.register();
    const errorPromise = new Promise<Error>((resolve) => emitter.once('error', resolve));
    (rspEngine as any).r2r.execute = async () => {
        throw new Error('synthetic query failure');
    };

    stream?.add(new Set([makeQuad('error-0')]), 0);
    stream?.add(new Set([makeQuad('error-10')]), 10);

    await expect(errorPromise).resolves.toEqual(new Error('synthetic query failure'));
});

test('centered semantics expose one logical timestamp for every binding in a window', async () => {
    const rspEngine = new RSPEngine(singleWindowQuery('10 STEP 10'), { window_semantics: 'centered' });
    const stream = rspEngine.getStream("https://rsp.js/stream1");
    const emitter = rspEngine.register();
    const resultsPromise = waitForResults(emitter, 2);

    stream?.add(new Set([makeQuad('centered-0')]), 0);
    stream?.add(new Set([makeQuad('centered-1')]), 1);
    stream?.add(new Set([makeQuad('centered-10')]), 10);
...
test('exact boundaries use adjacent half-open windows', async () => {
    const rspEngine = new RSPEngine(singleWindowQuery('10 STEP 10'));
    const stream = rspEngine.getStream("https://rsp.js/stream1");
    const emitter = rspEngine.register();
    const resultsPromise = waitForResults(emitter, 2);

    stream?.add(new Set([makeQuad('boundary-0')]), 0);
    stream?.add(new Set([makeQuad('boundary-10')]), 10);
    stream?.add(new Set([makeQuad('boundary-20')]), 20);
...
test('correlates multiple streams by logical window bounds with out-of-order input', async () => {
    const rspEngine = new RSPEngine(twoWindowJoinQuery(), { max_delay: 5 });
    const stream1 = rspEngine.getStream("https://rsp.js/stream1");
    const stream2 = rspEngine.getStream("https://rsp.js/stream2");
    const emitter = rspEngine.register();
    const resultsPromise = waitForResults(emitter, 2);
    ...
    stream1?.add(new Set([quad(
        namedNode(subject),
        namedNode('https://rsp.js/p1'),
        namedNode('https://rsp.js/left'),
        defaultGraph(),
    )]), 0);
    stream2?.add(new Set([quad(
        namedNode(subject),
        namedNode('https://rsp.js/p2'),
        namedNode('https://rsp.js/on-time'),
        defaultGraph(),
    )]), 0);
    stream1?.add(new Set([makeQuad('join-left-10', 'https://rsp.js/p1', 'https://rsp.js/left-10')]), 10);
    stream2?.add(new Set([quad(
        namedNode(subject),
        namedNode('https://rsp.js/p2'),
        namedNode('https://rsp.js/on-time-10'),
        defaultGraph(),
    )]), 10);
    stream2?.add(new Set([quad(
        namedNode(subject),
        namedNode('https://rsp.js/p2'),
        namedNode('https://rsp.js/late'),
        defaultGraph(),
    )]), 5);
    stream1?.add(new Set([makeQuad('join-left-15', 'https://rsp.js/p1', 'https://rsp.js/left-15')]), 15);
