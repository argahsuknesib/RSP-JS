...
function makeQuad(id: string, predicate = 'http://rsp.js/test_property') {
    return quad(
...
test('preserves upstream tick mode distinctions', () => {
    const reportCounts = [Tick.TimeDriven, Tick.TupleDriven, Tick.BatchDriven].map((tick) => {
        const window = new CSPARQLWindow(':window1', 10, 10, ReportStrategy.OnWindowClose, tick, 0);
        const reports: QuadContainer[] = [];
...
test('reports content changes without duplicate watermark reports', () => {
    const window = new CSPARQLWindow(':window1', 10, 10, ReportStrategy.OnContentChange, Tick.TimeDriven, 0);
    const reports: QuadContainer[] = [];
    window.subscribe('RStream', (content) => reports.push(content));
...
test('accepted out-of-order content changes are reported once', () => {
    const window = new CSPARQLWindow(':window1', 10, 10, ReportStrategy.OnContentChange, Tick.TimeDriven, 0, 5);
    const reports: QuadContainer[] = [];
    window.subscribe('RStream', (content) => reports.push(content));
...
] as Array<[number, string[]]>)('keeps window origin at zero for first event %i', (timestamp, expectedDefinitions) => {
