...
import * as ENGINE_CONFIG from "./config/log_engine.json";
...
export class RSPEngine {
    ...
    constructor(query: string, opts?: {
        ...
        parsed_query.s2r.forEach((window: WindowDefinition) => {
            const windowImpl = new CSPARQLWindow(window.window_name, window.width, window.slide, ReportStrategy.OnWindowClose, Tick.TimeDriven, 0, this.max_delay, ENGINE_CONFIG.trigger_threshold);
